...
($\beta_{i}$) that needs to be determined. In a generalized linear model
(GLM), an offset specification looks like:

$$\widehat{Y} = g^{- 1}\left( offset + \beta_{0} + \beta_{1}X_{1} + \beta_{2}X_{2} + \ldots + \beta_{p}X_{p} \right)$$
...
- [`poisson_reg_offset()`](https://mattheaphy.github.io/offsetreg/reference/poisson_reg_offset.md) -
  create a Poisson GLM spec. Engines:
...
- [`boost_tree_offset()`](https://mattheaphy.github.io/offsetreg/reference/boost_tree_offset.md) -
  ...
  - `xgboost_offset` - a wrapper around
...
- [`decision_tree_exposure()`](https://mattheaphy.github.io/offsetreg/reference/decision_tree_exposure.md) -
  create a Poisson decision tree with weighted exposures. Engines:

  - `rpart_exposure` - a wrapper around
...
Verify that coefficients match
[`stats::glm()`](https://rdrr.io/r/stats/glm.html).
